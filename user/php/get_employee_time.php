...
$employee = $conn->real_escape_string($_GET['employee']);
$date = $_GET['date']; 
$duration = intval($_GET['duration']); 

$sql = "SELECT start_time, end_time FROM employees WHERE name = '$employee' LIMIT 1";
...
if ($result && $result->num_rows > 0) {
    ...
    $empIdQuery = "SELECT employee_id FROM employees WHERE name = '$employee' LIMIT 1";
    $empResult = $conn->query($empIdQuery);
    $employeeId = 0;
