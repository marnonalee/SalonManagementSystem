      ...
      <section class="mt-6">
          ...
          <table class="min-w-full text-sm text-left text-gray-700">
            ...
            <tbody class="divide-y divide-gray-200 bg-white text-sm">
              <?php if ($result && $result->num_rows > 0): ?>
                <?php $serial = $offset + 1; while ($row = $result->fetch_assoc()): ?>
                  <tr>
                    <td class="px-4 py-2 text-sm text-center"><?php echo $serial++; ?></td>
                    <td class="px-4 py-2 text-sm text-center"><?php echo htmlspecialchars($row['user_name'] ?? 'N/A'); ?></td>
                    <td class="px-4 py-2 text-sm text-center"><?php echo htmlspecialchars($row['service_name'] ?? 'N/A'); ?></td>
                ...
                <?php endwhile; ?>
              <?php else: ?>
                <tr><td colspan="7" class="text-center px-4 py-4 text-sm text-gray-500">No payment records found.</td></tr>
              <?php endif; ?>
            </tbody>
          </table>
        </div>
        ...
        <div class="mt-6 flex justify-center">
          <nav class="inline-flex rounded-md shadow-sm" aria-label="Pagination">
            <?php if ($page > 1): ?>
              <a href="?page=<?php echo $page - 1; ?>" class="px-3 py-1 border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-200">Previous</a>
            <?php endif; ?>
            <?php for ($i = 1; $i <= $total_pages; $i++): ?>
              <a href="?page=<?php echo $i; ?>" class="px-3 py-1 border border-gray-300 text-sm font-medium <?php echo $i == $page ? 'bg-gray-300' : 'text-gray-700 hover:bg-gray-200'; ?>"><?php echo $i; ?></a>
            <?php endfor; ?>
            <?php if ($page < $total_pages): ?>
              <a href="?page=<?php echo $page + 1; ?>" class="px-3 py-1 border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-200">Next</a>
            <?php endif; ?>
          </nav>
        </div>
      </section>
  ...
  <script>
    ...
    function openModal(src) {
      ...
      modal.innerHTML = `<div class="relative">
          ...
          <button onclick="this.parentElement.parentElement.remove()" class="absolute top-0 right-0 m-2 text-white text-xl font-bold">&times;</button>
        </div>`;
